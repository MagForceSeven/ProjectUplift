...
#include "UpliftCampaignSaveBlueprintUtilities.generated.h"

// Blueprint utility for asynchronously enumerating the collection of save games
UCLASS( )
class CAMPAIGN_API UEnumerateSaveGameHeaders_AsyncAction : public UBlueprintAsyncAction_SF
{
	GENERATED_BODY( )
public:
	// Delegate type/params for operation completion
	DECLARE_DYNAMIC_MULTICAST_DELEGATE_TwoParams( FCompletionDelegate, int, UserIndex, const TArray< FEnumeratedSaveGameHeader >&, Headers );
	// Delegate type that can be used to filter the save game headers
	DECLARE_DYNAMIC_DELEGATE_RetVal_FourParams( bool, FSaveGameFilter, const FString&, SlotName, int32, UserIndex, const UUpliftCampaignSaveHeader*, Header, ESaveDataLoadResult, LoadingResult );
...
// Blueprint utility for asynchronously enumerating the collection of save games
UCLASS( )
class CAMPAIGN_API UForEachSaveGameHeaders_AsyncAction : public UBlueprintAsyncAction_SF
{
	GENERATED_BODY( )
public:
	// Delegate type/params for each header that is found
	DECLARE_DYNAMIC_MULTICAST_DELEGATE_ThreeParams( FEnumHeader, const FString&, SlotName, const UUpliftCampaignSaveHeader*, Header, ESaveDataLoadResult, LoadingResult  );
	// Delegate type/params for operation completion
...
UCLASS( )
class CAMPAIGN_API ULoadSaveGame_AsyncAction : public UBlueprintAsyncAction_SF
{
	GENERATED_BODY( )
public:
	// Delegate type/params for operation completion
...
private:
	...
	void AsyncLoadComplete( const FString &AsyncSlotName, int32 AsyncUserIndex, ESaveDataLoadResult Result, const UUpliftCampaignSaveHeader *Header, const UUpliftCampaignSave *SaveData );
};
...
UCLASS( )
class CAMPAIGN_API USaveSaveGame_AsyncAction : public UBlueprintAsyncAction_SF
{
	GENERATED_BODY( )
public:
...
private:
	...
	UPROPERTY( )
	TObjectPtr< const UUpliftCampaignSave > Checkpoint = nullptr;
...
UCLASS( )
class UCreateCheckpointSave_AsyncAction : public UBlueprintAsyncAction_SF
{
	GENERATED_BODY( )
public:
	// Delegate type for signals for the completion of this action
	DECLARE_DYNAMIC_MULTICAST_DELEGATE_TwoParams( FCompletionDelegate, bool, Success, const UUpliftCampaignSave*, CheckpointData );
...
private:
	...
	void AsyncCheckpointComplete( const UObject *WorldContext, const UUpliftCampaignSave *CheckpointData, bool Success );
};